...
v_bounds_tight.v_upper = 1.01.^2*ones(Nb, 1);
v_bounds_tight.v_lower = 0.99.^2*ones(Nb, 1);

v_bounds_loose = struct();
v_bounds_loose.v_upper = 1.02.^2*ones(Nb, 1);
v_bounds_loose.v_lower = 0.98.^2*ones(Nb, 1);

v0_bounds = struct();
v0_bounds.v_upper = 1.01.^2;
v0_bounds.v_lower = 0.99.^2;

%%
load_max_pf = 0.85; load_phi = tan(acos(load_max_pf));
tnomi_p_load = 1; %how many times the nominal load is the mean
stdev_p_load = 0.3; %standard deviation of the random var
...
n_rlz = 150; % number of realizations of the random vars
hyp.seed = 20;
...
hyp.epsilon0_p0 = 1/50;
hyp.epsilon0_v0 = 0.0001;
hyp.epsilon0_pd = 1/1000;
hyp.mu0         = 400;
...
results = stochastic_solver_avg(benchmark, ...
    first_stage_initial, nu_lower_initial, nu_upper_initial, ...
    random_vars, params, ...
